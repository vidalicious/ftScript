    ...
    while True:
        ...
        if counter > windowCount:
            ...
            if hasBullPosition:
                ...
                elif floatPrice(bullSell1Price) - floatPrice(currentTarget) > floatPrice(currentTarget) - floatPrice(bullBuy1Price):
                ...
                if abs(floatPrice(currentIndex) - indexRecyclePrice) < 300:
                    ...
                    if floatPrice(bullSell1Price) - floatPrice(currentTarget) > floatPrice(currentTarget) - floatPrice(bullBuy1Price):
                        tradePrice = bullBuy1Price
                        pathTag.append(" sss ")
                        print "sss"
                        simu_checkOrderAndBuyWith(connectSocket, tradePrice, tradeOneHand, bullCode, file, pathTag)

                    # if lastMean10s < lastMean1 and mean10s > mean1:
                    #     tradePrice = bullBuy1Price
                    #     pathTag.append(" 6 ")
                    #     print "f"
                    #     simu_checkOrderAndBuyWith(connectSocket, tradePrice, tradeOneHand, bullCode, file, pathTag)
