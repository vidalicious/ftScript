...
if connectSocket is not None:
    while True:
        ...
        file = open("bear data", "a+")
        
        currentBearPrice = getCurrentPrice(connectSocket, bearCode)
        print "counter ", str(counter), " bear ", str(floatPrice(currentBearPrice)), " time ", time.strftime('%Y-%m-%d %H:%M:%S'), " bear code ", bearCode
        logger = ["counter ", str(counter), " bear ", str(floatPrice(currentBearPrice)), " time ", time.strftime('%Y-%m-%d %H:%M:%S'), " bear code ", bearCode, "\n"]
        file.writelines(logger)
        
        priceList.insert(0, floatPrice(currentBearPrice))
        if len(priceList) > windowCount:
            priceList = priceList[:windowCount]

        if mean1 != 0:
            mean1 = floatPrice(currentBearPrice) * ema1_K + mean1 * (1 - ema1_K)
        else:
            mean1 = floatPrice(currentBearPrice)

        if mean5 != 0:
            mean5 = floatPrice(currentBearPrice) * ema5_K + mean5 * (1 - ema5_K)
        else:
            mean5 = floatPrice(currentBearPrice)
